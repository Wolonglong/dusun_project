...
for FILE in "${FILES[@]}"; do
    ...
    echo "REL_DIR: $REL_DIR"
		...
		if [[ ! -d "$PROJECT_ALL_DIR/$PROJECT_NAME/$PROJECT_RELEASE/$REL_DIR" ]]; then
				mkdir -p "$PROJECT_ALL_DIR/$PROJECT_NAME/$PROJECT_RELEASE/$REL_DIR"
		fi

    cp -r "$ABS_PATH" "$PROJECT_ALL_DIR/$PROJECT_NAME/$PROJECT_RELEASE/$REL_DIR"
		ls -l $PROJECT_ALL_DIR/$PROJECT_NAME/$PROJECT_RELEASE/$REL_PATH
done
